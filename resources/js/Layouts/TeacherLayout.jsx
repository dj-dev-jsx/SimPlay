...
export default function TeacherLayout({ children }) {
  ...
  const { component } = usePage(); // Current Inertia component
  ...
  const navItems = [
    {
      icon: Home,
      label: "Dashboard",
      href: route("teacher.teacher_dashboard"),
      componentName: "Teacher/Dashboard",
    },
    {
      icon: BookOpen,
      label: "Classes",
      href: route("teacher.teacher_classes"),
      componentName: "Teacher/Classes",
    },
    {
      icon: Paperclip,
      label: "Activities",
      href: route("teacher.teacher_activities"),
      componentName: "Teacher/Activities",
    },
    {
      icon: Users,
      label: "Teachers",
      href: route("teacher.teachers"),
      componentName: "Teacher/Teachers",
    },
  ];

  // Check if nav item is active
  const isActive = (item) => item.componentName === component;
        ...
        <nav className="flex-1 px-6 py-6 space-y-3">
          {navItems.map((item) => (
            <Link
              ...
              href={item.href}
              className={`
                flex items-center gap-3 px-5 py-3 rounded-2xl shadow-md font-semibold text-base sm:text-lg
                transition-all duration-200
                ${isActive(item)
                  ? "bg-green-500 text-white" // active
                  : "bg-green-800 hover:bg-green-600 text-white"} // default
              `}
            >
